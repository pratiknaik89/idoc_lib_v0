...
import { Control, Extras, Dataset, Style } from './model/model'
...
export class iDocsignviewerComponent implements OnInit {
    ...
    getValues(recipient = undefined) {
        ...
        this.loopEachControl((control: Control) => {
            ...
            if (recipient) {
                if (_rec === recipient)
                    values[control.id] = { 'val': control.val }
            } else {
                values[control.id] = { 'val': control.val }
            }
    ...
    validate(recipient = undefined) {
        let values = [];
        this.loopEachControl((control: Control) => {
            if (control.dataset.require == true) {
                var _rec = '';
                if (control.extras && control.extras.recipient)
                    _rec = control.extras.recipient.val;
                var isrecepientmatch = _rec === recipient;
                var isvalue = (control.val && control.val.trim() != "");
                if (recipient && isrecepientmatch && !isvalue) {
    ...
    loopEachControl(callback) {
        ...
        for (let i = 0; i < pages.length; i++) {
            ...
            for (let j = 0; j < ele.length; j++) {
                ...
                if (propele.type == "checkbox" || propele.type == "radio") {
                    if (propele.dataset.groupids) {
